...
def tv_cgls(initial, max_inner, max_outer, A, sino, alpha, er):
    uk = initial.copy()
    ...
    for k in range(max_outer):
        ...
        uk_grad = grad[0]**2 + grad[1]**2 + grad[2]**2
        uk_grad_np = uk_grad.as_array()
        ...
        # reweight large values
        uk_reweighted_np = np.zeros_like(uk.as_array()) # small terms are automatically put to zero
        uk_reweighted_np[large_values] = uk_grad_np[large_values] ** (-1/4)
        ...
        zero_operator = ZeroOperator(ig)

        # reg_operator = [W^{1/2}D_x W^{1/2}D_y]^T
        w_matrix = BlockOperator(W_diagonal, zero_operator, zero_operator, zero_operator, 
                                 W_diagonal, zero_operator, zero_operator, zero_operator, W_diagonal, shape=(3,3))
        ...
        cgls.run(max_inner, verbose=0, callbacks=[callbacks.CGLSEarlyStopping(1e-2)])
...
def apply_operator_onenorm(x, W, alpha):
...
def CG_onenorm(initial, iteration, b, alpha, Wk):
    ...
    u0 = initial.copy()
    r0 = b - apply_operator_onenorm(u0, Wk, alpha)
    p0 = r0
    ...
    for k in range(iteration):
        zk = apply_operator_onenorm(pk, Wk, alpha)
...
def onenorm_normal(max_outer, max_inner, initial, A, b, alpha, er):
    ...
    for k in range(max_outer):
        ...
        # performing CG
        cg_output = CG_onenorm(uk, max_inner, lhs, alpha, uk_reweighted)
...
def apply_operator_tv(weights_k, alpha, x):
    ...
    z2_c = diagonal_weights.direct(z1[2])
    z2 = BlockDataContainer(z2_a, z2_b, z2_c)
...
def CG_tv(initial, iteration, b, alpha, Wk):
    ...
    u0 = initial.copy()
    r0 = b - apply_operator_tv(weights_k=Wk, alpha=alpha, x=u0)
    p0 = r0
    ...
    for k in range(iteration):
        ...
        # zk = Apk
        zk = apply_operator_tv(weights_k=Wk, alpha=alpha, x=pk)
...
def tv_solver_normal(initial, inner, outer, A, b, alpha, er):
    """
    Finds a solution for the problem 
