        ...
        if self.roi != None:
            for key in self.roi.keys():
                ...
                if (isinstance(data, ImageData)):
                    ...
                    if key == 'channel':
                        geometry.set_channels(num_channels = int(np.ceil((roi[idx][1] - roi[idx][0]) / roi[idx][2])))
                    elif key == 'horizontal':
                        geometry.config.panel.num_pixels[0] = int(np.ceil((roi[idx][1] - roi[idx][0]) / roi[idx][2]))
                        geometry.config.panel.pixel_size[0] = geometry_0.config.panel.pixel_size[0]
                    elif key == 'vertical':
                        geometry.config.panel.num_pixels[1] = int(np.ceil((roi[idx][1] - roi[idx][0]) / roi[idx][2]))
                        geometry.config.panel.pixel_size[1] = geometry_0.config.panel.pixel_size[1]
                    elif key == 'angle':
                        geometry.set_angles(geometry_0.angles[sliceobj[idx]],
